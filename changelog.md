...
- feat: Library settings can add extra folders (manga, books, or both, with a max-depth ceiling), optionally scan Default Location, **Scan now** across those roots, scan on start, and interval scans. Watch is listed but not live yet. Opening a one-shot image folder keeps that folder as the library item. Moving a series then opening or locating the new path can update the old row, or merge if a duplicate already exists at the new path. Scan now still locks the window; start and interval scans stay in the background with a title-bar status.
