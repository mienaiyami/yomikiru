...
const pushRelease = () => {
    ...
    changelog = changelog
        .replaceAll("$$TAG$$", pkgJSON.version)
    ...
    if (fs.existsSync("./changelogTemp.md")) fs.rmSync("./changelogTemp.md");
    fs.writeFileSync("./changelogTemp.md", changelog);
    const pushCommand =
        `gh release create v${pkgJSON.version} -t v${pkgJSON.version} ` +
    ...
    ghSpawn.on("close", (code) => {
        ...
        if (code === 0) {
            console.log(
                "\x1b[92mPushed release to github. https://github.com/mienaiyami/yomikiru/releases.\x1b[0m"
            );
...
const makeExe = () => {
    ...
    yarnSpawn.on("close", (code) => {
        ...
        if (fs.existsSync(`./out/make/squirrel.windows/ia32/Yomikiru-${pkgJSON.version} Setup.exe`)) {
            ...
            fs.rename(
                `./out/make/squirrel.windows/ia32/Yomikiru-${pkgJSON.version} Setup.exe`,
                `./out/full/Yomikiru-${pkgJSON.version}-Setup.exe`,
                (err) => {
...
const makeZip = () => {
    ...
    yarnSpawn.on("close", (code) => {
        ...
        if (fs.existsSync(`./out/make/zip/win32/ia32/Yomikiru-win32-ia32-${pkgJSON.version}.zip`)) {
            ...
            fs.rename(
                `./out/make/zip/win32/ia32/Yomikiru-win32-ia32-${pkgJSON.version}.zip`,
                `./out/full/Yomikiru-win32-${pkgJSON.version}-Portable.zip`,
                (err) => {
                    if (err) return console.error(err);
                    console.log("\x1b[92mmoved successfully.\x1b[0m");
                    fs.writeFileSync(
