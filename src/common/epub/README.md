...
XML is parsed with `fast-xml-parser` in `xml.ts`, then adapted to the small namespace-agnostic tree API used by the package readers. XML is validated before conversion; DTD processing remains disabled. Spine hrefs are indexed once before NCX/nav processing so large tables of contents do not repeatedly scan the spine. Tests cover namespace-prefixed metadata, nested nav text, multi-token manifest properties, malformed XML, chapter references and event handlers, and `meta[name=cover]`.
