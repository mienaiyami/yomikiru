...
Full-page replacement of the gallery grid (not a side drawer). Shared hero: cover with overlay back, title (EPUB badge on books), optional author, Continue/Start plus icon favourite / edit metadata / Select Cover / Show in File Explorer / Copy Path, compact AniList when a token exists. Right-click the cover for the gallery tile menu plus **Show cover in File Explorer** (the displayed image file). Opening the page focuses Continue/Start. The details-list toolbar toggles `galleryDetailsHeroLayout` between the existing vertical header and a horizontal metadata sidebar; the choice is shared by manga/book and persisted. Vertical mode keeps cover/title sticky and uses `galleryDetailsHeroHeight`. Horizontal mode uses `galleryDetailsHeroWidth`, a portrait cover, local progress and tags beside it, then the item note and tracker-derived metadata below; only the cover Back control and title remain sticky. The compact AniList control stays with Continue. Each dimension uses `0` for CSS auto sizing and is remembered independently when its divider is dragged. When a tracker snapshot has a cover image, details and tiles use it unless `library_items.extra.detailsCoverSource` is `"library"`. The Cover: Default / Cover: AniList toggle next to Track writes that key. Current chapter sits with last-read date, manga page, and chapters-read (`read / total`); when the tracker media snapshot has releasing status, score, chapter count, or format, that line sits above genres with a hairline between them (hidden when those fields are empty; no Tracker label). Genres sit above About when resolved metadata supplies them (hidden when empty). Click the title **Note** to edit; Escape or blur commits to `library_items.note`, and height follows the note text. Lists sit under the same `galleryToolbar` chrome as gallery home (tabs left; locate-current / sort/refresh left of search on gallery home and details only). **Directory Up** (`dirUp`) closes details and focuses gallery search (including after returning from the reader; window capture; ignored while the reader is open, while Settings or AniList overlays are open, and while typing in a field). Context-menu shortcut (`ctrl+/` by default) works on focused tiles and details rows the same way as classic lists.
