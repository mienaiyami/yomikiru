...
import { createTestFs } from "@test/mocks/preload";
 ...
 */
const makeService = (fsOverrides: Parameters<typeof createTestFs>[0] = {}) => {
    const logger = {
        log: vi.fn(),
        error: vi.fn(),
        warn: vi.fn(),
    ...
    return new DirectoryValidatorService({
        ...
        app: window.app,
        onProgress: vi.fn(),
        appSettings: { keepExtractedFiles: false, pdfScale: 1.5 },
    });
};

describe("DirectoryValidatorService", () => {
    it("cancel is not implemented yet", () => {
        expect(() => makeService().cancel()).toThrow(/Not implemented/);
    });
